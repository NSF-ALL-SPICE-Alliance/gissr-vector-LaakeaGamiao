...
```{r}
  # Note we have to rearrange the coordinates of the bounding box a little bit 
  # to give get_inat_obs() what it expects
inat_obs_df <- get_inat_obs(taxon_id = 320058, #taxon id from URL
                            place_id = 7316, #place id from URL
                            quality = "research", #specifies research grade only
                            geo = TRUE, #Specifies that we want geocoordinates
                            maxresults = 100) #Limits results

save(inat_obs_df, file = "guam_cocorhino_inat.Rdata")
```
...
```{r}
# retrieving data of streets on Guam
guam_streets <- guam_bb %>%
  opq() %>%
  add_osm_feature("highway", c("motorway", "primary", "secondary", "tertiary")) %>%
  osmdata_sf()

# retrieving data of small streets on Guam
guam_small_streets <- guam_bb %>%
  opq() %>%
  add_osm_feature(key = "highway", value = c("residential", "living_street", "unclassified", "service", "footway")) %>%
  osmdata_sf()

# retrieving data of coastline on Guam
guam_coast <- guam_bb %>%
  opq() %>%
  add_osm_feature(key = "natural", value = "coastline") %>%
  osmdata_sf()
...
p <- ggplot() +
  ...
  coord_sf(xlim = c(144.56343, 145.00917), ylim = c(13.18234, 13.70618), expand = TRUE) + # setting the limits of our map based on the lat/long we got from our OSM bounding box
  ggtitle("Coconut Rhinoceros Beetles on Guam", subtitle = "Based on iNaturalist Data as of September 2024") +
  theme_bw() +
  labs(
    color="Common Name", #this changes the title of our legend
    x = "Longitude",
...
```{r}
htmltools::p("iNaturalist Observations of Rhinoceros Beetles on Guam",
             htmltools::br(),
             inat_obs_popup_sf$datetime %>% 
               as.Date() %>% 
               range(na.rm = TRUE) %>% 
...
```{r}
leaflet(inat_obs_sf) %>% 
  setView(lng = 144.78630, lat = 13.44426, zoom = 12)%>%
  addTiles() %>% 
  addCircleMarkers(data = inat_obs_popup_sf,
                   popup = ~popup_html, 
                   radius = 5)
