...
ggplot() +
    geom_sf(data = world) +
    coord_sf(xlim = c(135, 141), ylim = c(34.5, 37.5), expand = FALSE) +
  ggtitle("Central Japan") +
  theme_void()
```
...
```{r}
states <- (ne_states(country = "Japan"))
```
...
```{r}
ggplot() +  #Got rid of world shapefile because it made a weird double line
    geom_sf(data = states) +
...
ggplot() +
    ...
    geom_sf(data = lakes50, fill = "blue", color = "black") +
...
ggplot() +
    ...
    geom_sf(data = rivers50, color = "lightblue", size = 0.15) +
    geom_sf(data = tokyo, fill = "antiquewhite2", color = "red")+
    geom_text(data = states, aes(x = longitude, y = latitude, label = name), size = 3, fontface = "bold", nudge_y = states$nudge_y) +
    coord_sf(xlim = c(135, 141), ylim = c(34.5, 37.5), expand = FALSE) +
    ggtitle("Tokyo", subtitle = "in the Central Region of Japan") +
    theme_void()
```
