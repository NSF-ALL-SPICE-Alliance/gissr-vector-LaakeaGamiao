...
```{r}
#bb <- getbb("Maui")
#bb <- getbb("Japan") #Ran into issues with it being too big
bb <- getbb("Guam")
```
...
ggplot() +
  ...
  geom_sf_text(data = schools$osm_polygons, aes(label = name), size = 2, hjust = 0) + #Used int_name because it doesnt register japanese characters and gives blank boxes
  coord_sf(xlim = c(144.61, 145.1), ylim = c(13.3, 13.7), expand = FALSE)
```
...
```{r}
#Retrieving data of streets in Guam
streets <- bb %>%
  opq() %>%
  add_osm_feature("highway", c("motorway", "primary", "secondary", "tertiary")) %>%
  osmdata_sf()

#Retrieving data of small streets in Guam
small_streets <- bb %>%
  opq() %>%
  add_osm_feature(key = "highway", value = c("residential", "living_street", "unclassified", "service", "footway")) %>%
  osmdata_sf()

#Retrieving data of coastline in Guam
coastline <- bb %>%
  opq() %>%
...
ggplot() +
  ...
  geom_sf(data = coastline$osm_lines, inherit.aes = FALSE, color = "black", size =.8, alpha = 0.5) +
  geom_sf(data = schools$osm_polygons, inherit.aes = FALSE, colour = "red", fill = "red", alpha = 0.5, size = 1) +
  coord_sf(xlim = c(144.61, 145.05), ylim = c(13.2, 13.7), expand = FALSE) +
  geom_sf_text(data = schools$osm_polygons, aes(label = name), 
               size = 2, 
               hjust = 0, 
               nudge_y = 0.01,  # Adjust vertical position to avoid overlap
               check_overlap = TRUE) +  # Prevent overlap of labels
  theme_bw() +
  labs(
    title = "Schools in Guahan",
    x = "Longitude",
    y = "Latitude"
  )
```
